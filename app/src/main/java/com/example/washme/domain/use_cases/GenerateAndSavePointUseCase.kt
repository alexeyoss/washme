...
@Inject constructor(
    ...
    suspend operator fun invoke(amount: Int, location: UserLocation): Flow<CommonStates> {
        return flow {
            emit(CommonStates.Loading)
            val job = scope.async {
                coroutineScope {
                    val washMePoints = pointRepository.generateRandomPoints(amount, location)
                    pointRepository.savePointIntoDb(washMePoints)
                }
